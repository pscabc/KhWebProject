...
public class TouristServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        urlBuilder.append("&" + URLEncoder.encode("contentTypeId","UTF-8") + "=" + "12"); /*(A=제목순,B=조회순,C=수정순,D=생성일순) 대표이미지가 반드시 있는 정렬 (O=제목순, P=조회순, Q=수정일순, R=생성일순)*/
        urlBuilder.append("&" + URLEncoder.encode("areaCode","UTF-8") + "=" + "1"); /*(A=제목순,B=조회순,C=수정순,D=생성일순) 대표이미지가 반드시 있는 정렬 (O=제목순, P=조회순, Q=수정일순, R=생성일순)*/
        ...
        RequestDispatcher red = request.getRequestDispatcher("/WEB-INF/views/touristBoard.jsp");
