package org.yeodam.tourist.controller.load;
...
/**
 * Servlet implementation class LoadTouristServlet
 */
@WebServlet(name = "LoadTourist", urlPatterns = { "/loadTourist" })
public class LoadTouristServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public LoadTouristServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        urlBuilder.append("&" + URLEncoder.encode("areaCode","UTF-8") + "=" + "1"); /*(A=제목순,B=조회순,C=수정순,D=생성일순) 대표이미지가 반드시 있는 정렬 (O=제목순, P=조회순, Q=수정일순, R=생성일순)*/
        urlBuilder.append("&" + URLEncoder.encode("contentTypeId","UTF-8") + "=" + typeId); /*(A=제목순,B=조회순,C=수정순,D=생성일순) 대표이미지가 반드시 있는 정렬 (O=제목순, P=조회순, Q=수정일순, R=생성일순)*/
        urlBuilder.append("&" + URLEncoder.encode("areaCode","UTF-8") + "=" + "1"); /*(A=제목순,B=조회순,C=수정순,D=생성일순) 대표이미지가 반드시 있는 정렬 (O=제목순, P=조회순, Q=수정일순, R=생성일순)*/
        ...
        for(int i = 0; i < detail3.size(); i++) {
        	...
        	touristMap.put("touristConId",detail3.get(i).getAsJsonObject().get("contenttypeid").getAsString());
        	touristMap.put("touristId",detail3.get(i).getAsJsonObject().get("contentid").getAsString());
