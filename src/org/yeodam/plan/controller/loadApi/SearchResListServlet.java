...
/**
 * Servlet implementation class SearchResListServlet
 */
@WebServlet(name = "SearchResList", urlPatterns = { "/searchResList" })
public class SearchResListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchResListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        urlBuilder.append("&" + URLEncoder.encode("contentTypeId","UTF-8") + "=" + category); /*(A=제목순,B=조회순,C=수정순,D=생성일순) 대표이미지가 반드시 있는 정렬 (O=제목순, P=조회순, Q=수정일순, R=생성일순)*/
...
//        System.out.println("Response code: " + conn.getResponseCode());
        ...
        for(int i = 0; i < detail3.size(); i++) {
        	...
        	touristMap.put("touristImg",detail3.get(i).getAsJsonObject().has("firstimage") ? detail3.get(i).getAsJsonObject().get("firstimage").getAsString() : "/img/사진없음.jpg");
